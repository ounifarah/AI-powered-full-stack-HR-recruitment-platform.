...
import { UserCircle, CalendarDays, AlertTriangle, Hexagon } from 'lucide-react';

const Sidebar = ({ activeTab, setActiveTab }) => {
    const sidebarItems = [
        { name: 'My Profile', icon: <UserCircle className="w-5 h-5" /> },
        { name: 'Request Leave', icon: <CalendarDays className="w-5 h-5" /> },
        { name: 'Report Conflict', icon: <AlertTriangle className="w-5 h-5" /> },
    ];
        ...
        <aside className="w-[300px] bg-slate-950 border-r border-slate-900 flex flex-col h-screen z-20 font-sans shadow-2xl relative">
            <div className="p-8 pb-4 h-full flex flex-col relative z-10">
                <div className="flex items-center gap-4 mb-12 pb-8 border-b border-slate-800/50">
                    ...
                    <div>
                        <h1 className="text-xl font-black tracking-tight text-white">ApexIT Employee</h1>
                        <p className="text-[10px] text-blue-400 font-bold tracking-widest mt-1 uppercase">Employee Space</p>
                    </div>
                </div>
